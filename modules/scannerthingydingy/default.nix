...
# Configure Scanner Key

    ## 1 Keypress start Duplex

    ## 2 Presses start backside duplex
        ### when finished wait for next scanjob

                # if next job has also 2 two presses add next job to scanjob

                # if next scan has only one press add next scan and scanjob to their own files
                # each

                # if there is no next press for 5 minutes save scanjob as it is



## At buttonpress
    ### start Scansoftware
    scan2pdf --device "" --mode ""

    ### pipe to

###
# Scanpipeline to file
